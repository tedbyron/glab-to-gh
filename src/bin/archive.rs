...
//! Archive GitLab projects
//!
//! Usage: archive [OPTIONS] <--repo <REPO>|--su|--sg <GROUP>>
//!
//! Options:
...
//! ## Example
//!
//! Archive all repos in the `example-group` group:
//!
//! ```sh
//! cargo run --bin archive -- --sg example-group
//! #                            └source group
//! ```
...
#[derive(Debug, Parser)]
#[command(about = "Archive GitLab projects")]
struct CliArgs {
...
async fn run() -> Result<()> {
    ...
    // Loop through projects and archive each.
    for p in &projects {
        let endpoint = projects::ArchiveProject::builder()
            .project(&*p.path_with_namespace)
            .build()?;
        let p: Result<Project> = endpoint.query_async(glab.as_ref()).await.map_err(|e| {
            anyhow!(
                "Couldn't archive the GitLab project: {}: {e}",
        ...
        if !p.archived {
            prompt_continue(anyhow!(
                "Couldn't archive the GitLab project: {}",
                p.path_with_namespace
            ))?;
            continue;
        }
    ...
    info!("{count}/{} projects archived", projects.len(),);
